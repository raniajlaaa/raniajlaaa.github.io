...
function switchTab(tabName) {
  ...
  document.querySelectorAll('.resume-slider').forEach(slider => slider.classList.remove('active'));

  document.querySelector(`.tab-btn[onclick*="${tabName}"]`).classList.add('active');
  document.querySelector(`.resume-slider.${tabName}`).classList.add('active');

  updateSlider(tabName);
}

function goToSlide(index, tab = 'experience') {
  currentSlides[tab] = index;
  updateSlider(tab);
}

function updateSlider(tab) {
  const track = document.querySelector(`.resume-slider.${tab} .slider-track`);
  const offset = -currentSlides[tab] * 100 / 3;
